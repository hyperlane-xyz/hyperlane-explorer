...
export function AddressInline({ address, chainName, startChars = 8, endChars = 6 }: Props) {
  ...
  const link = isZero ? null : getBlockExplorerAddressUrl(chainMetadataResolver, chainName, address);
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      {!isZero && (
        <CopyButton copyValue={address} width={12} height={12} className="opacity-60" />
      )}
      {link && (
